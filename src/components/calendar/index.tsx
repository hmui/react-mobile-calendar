...
> {
  ...
  touchStart = (event: any) => {
    ...
    this.setState({
      touchStartPositionX: event.touches ? event.touches[0].clientX : event.clientX,
      touchStartPositionY: event.touches ? event.touches[0].clientY : event.clientY,
      touch: { x: 0, y: 0 },
      isTouching: true,
      
      isMobile: !!event.touches
    });
  };
  ...
  touchMove = (event: any) => {
    ...
    let moveX = (event.touches ? event.touches[0].clientX : event.clientX) - touchStartPositionX;
    let moveY = (event.touches ? event.touches[0].clientY : event.clientY) - touchStartPositionY;
    ...
    if (Math.abs(touch.x) > Math.abs(touch.y) && Math.abs(touch.x) > 0.2) {
      this.setState({ currentChangeIsScroll: true }, () => {
        if (touch.x > 0) {
          ...
          if (isShowWeek) {
            setTimeout(() => {
              isMobile && this.setState({ isTouching: true });
              this.setState({ currentChangeIsScroll: true }, () => {
                this.getLastWeek();
              });
            }, transitionDuration * 1000);
